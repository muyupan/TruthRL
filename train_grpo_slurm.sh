#!/bin/bash
#SBATCH --job-name=model1_20step_32rollout
#SBATCH --account=
...
#SBATCH --mail-user=
#SBATCH --mail-type=BEGIN,END,FAIL,TIME_LIMIT_80,TIME_LIMIT_20
...
export OPENAI_API_BASE=http://$head_node_ip:8001/v1
# Print env for debugging
printenv

# # ----------------------------- Start Judge Server -----------------------------
# judge_node=${nodes_array[8]}
# judge_ip=$(srun --overlap --cpu-bind=none --nodes=1 --ntasks=1 -w "$judge_node" hostname --ip-address)

# echo "Starting Judge Server on $judge_node ($judge_ip)"
# srun --overlap --cpu-bind=none --nodes=1 --ntasks=1 -w "$judge_node" \
...
# export OPENAI_API_BASE=http://$judge_ip:8001/v1
# echo "Judge server at: $OPENAI_API_BASE"
...
#export MAX_JUDGE_CONCURRENCY=64

run_training() {
  PYTHONUNBUFFERED=1 srun --cpu-bind=none --overlap --nodes=1 --ntasks=1 -w "$head_node" --chdir=$verl_workdir \
...
while true; do
  ...
  if [[ $attempt -ge $MAX_RETRIES ]]; then
    ...
    cleanup_ray() {
      ...
      srun --cpu-bind=none --overlap --nodes=1 --ntasks=1 -w "$head_node" ray stop || true
      for ((i = 1; i <= worker_num; i++)); do
          node_i=${nodes_array[$i]}
