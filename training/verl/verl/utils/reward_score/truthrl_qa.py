...
def attempt_api_call(messages, max_retries=3):
    for attempt in range(max_retries):
        try:
            ...
            print(f"Attempt {attempt + 1} failed with Bad API key: {e}")
            if attempt < max_retries - 1:
                print(f"Retrying... ({attempt + 2}/{max_retries})")
            else:
                print(f"All {max_retries} attempts failed. Last error: Bad API key {e}")
                return None
        except litellm.RateLimitError as e:
            print(f"Attempt {attempt + 1} failed with Rate limited: {e}")
            if attempt < max_retries - 1:
                print(f"Retrying... ({attempt + 2}/{max_retries})")
            else:
                print(f"All {max_retries} attempts failed. Last error: Rate limited {e}")
                return None
            ...
            if attempt < max_retries - 1:
                ...
                print(f"All {max_retries} attempts failed. Last error: API error {e}")
                return None
        except Exception as e:
            print(f"Unexpected error: {e}")
            return None
